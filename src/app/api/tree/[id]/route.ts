...
export async function DELETE(
  ...
  try {
    ...
    if (node.user_id !== userId) {
      return NextResponse.json(
        { error: 'Anda hanya boleh menghapus node milik sendiri' },
        { status: 403 }
      );
    }

    // Cascade delete happens via FK constraints
    await client.query(`DELETE FROM nodes WHERE id = $1`, [nodeIdInt]);

    return NextResponse.json({ success: true, node_id: nodeIdInt });
  } catch (error) {
    console.error('Delete node error:', error);
    return NextResponse.json(
      { error: 'Gagal menghapus node' },
      { status: 500 }
    );
  } finally {
    client.release();
