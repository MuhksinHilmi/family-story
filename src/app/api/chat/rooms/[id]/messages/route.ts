...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const messagesRes = await pool.query(
      `SELECT id, chat_room_id, sender_id, sender_name_snapshot, body, is_system, created_at
       FROM messages 
    ...
    return NextResponse.json({
      messages: messagesRes.rows.map(row => ({
        ...
        sender: row.sender_name_snapshot ? { full_name: row.sender_name_snapshot } : null,
        content: row.body,
