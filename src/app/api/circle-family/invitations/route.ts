...
export async function GET(request: NextRequest) {
  try {
    ...
    const invitationsRes = await pool.query(
      `SELECT hi.id, hi.target_nuclear_family_id, hi.sender_node_id, hi.message, hi.created_at, hi.status,
              h.name as halaqah_name,
              s.full_name as inviter_name,
              sf.name as inviter_family
       FROM halaqah_invites hi
       LEFT JOIN halaqahs h ON hi.halaqah_id = h.id
       JOIN nodes sn ON hi.sender_node_id = sn.id
       JOIN nodes s ON sn.user_id = s.user_id
       LEFT JOIN nuclear_family_memberships snfm ON sn.id = snfm.node_id
    ...
    const invitations = invitationsRes.rows.map((inv: any) => ({
      id: inv.id,
      halaqah_id: inv.halaqah_id,
      halaqah_name: inv.halaqah_name || 'Halaqah Baru',
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!invitationId || !action) {
      return NextResponse.json({ error: 'ID undangan dan aksi diperlukan' }, { status: 400 });
    }
        ...
        for (const member of allMembers) {
          await pool.query(
            `INSERT INTO halaqah_members (halaqah_id, node_id, status, is_admin)
             VALUES ($1, $2, 'active', $3)`,
            [halaqah.id, member.node_id, member.node_id === (await pool.query(
