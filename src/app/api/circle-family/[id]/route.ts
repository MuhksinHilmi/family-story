...
export async function GET(
  ...
  try {
    ...
    const hRes = await pool.query("SELECT * FROM halaqahs WHERE id = $1", [
      halaqahId,
    ]);
...
export async function PATCH(
  ...
  try {
    ...
    if (learning_path_id) {
      updateFields.push(`learning_path_id = $${idx++}`);
      values.push(learning_path_id);
    }
    ...
    await pool.query(
      `UPDATE halaqahs SET ${updateFields.join(", ")} WHERE id = $${idx}`,
      values,
    );
