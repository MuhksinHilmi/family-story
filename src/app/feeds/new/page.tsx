...
export default function CreateFeedPage() {
  ...
  // Fetch extended contacts for custom picker
  useEffect(() => {
    if (scopeType === "custom") {
  ...
  const filteredContacts = extendedContacts.filter((p: any) =>
      ...
      <div className="mx-auto max-w-3xl px-4 lg:max-w-4xl">
        ...
        {step === 1 && (
          <div className="bg-white border border-[#D4C4A8] rounded-2xl p-6 space-y-6 mb-6 shadow-sm">
            <div>
              <div className="mb-3 text-sm font-medium text-[#6B5B45]">
                Pilih Cakupan Visibilitas
              </div>

              <div className="space-y-3">
                <button
                  type="button"
                  onClick={() => setScopeType("extended")}
                ...
                >
                  ...
                  <div className="text-sm text-[#6B5B45]">
                    Semua anggota yang terhubung melalui extended family group.
                  </div>
                </button>

                <button
                  type="button"
                  onClick={() => setScopeType("nuclear")}
                  className={`w-full rounded-xl border p-4 text-left transition ${
                    scopeType === "nuclear"
                      ? "border-[#4A7C59] bg-[#F5F0E8]"
                      : "border-[#D4C4A8] hover:bg-[#FDFAF5]"
                ...
                >
                  ...
                  <div className="text-sm text-[#6B5B45]">
                    Hanya anggota keluarga inti aktifmu saat ini (berdasarkan snapshot ayah).
                  </div>
                </button>
                ...
                >
                  ...
                  <div className="text-sm text-[#6B5B45]">
                    Pilih sendiri beberapa anggota dari keluarga besar.
                  </div>
                </button>
              </div>
            </div>
            ...
            {scopeType === "custom" && (
              <div className="border-t border-[#D4C4A8] pt-4">
                <div className="mb-2 text-sm font-medium text-[#6B5B45]">
                  Pilih anggota secara manual
                </div>
                  ...
                  {showMentionDropdown && mentionQuery.length > 0 && (
                    <div className="absolute z-50 mt-1 w-full rounded-xl border border-[#D4C4A8] bg-white shadow-lg max-h-60 overflow-auto">
                      {filteredContacts.slice(0, 8).map((person: any, index: number) => (
                        ...
                        >
                          <div className="h-8 w-8 flex-shrink-0 rounded-full bg-[#EDE4D3]" />
                          <span>{person.full_name}</span>
                        </button>
                      ))}
                      {filteredContacts.length === 0 && (
                        <div className="px-4 py-3 text-sm text-[#9C8B75]">
                          Tidak ada yang cocok di extended family Anda
                        </div>
                      )}
                    </div>
                  )}
                ...
                {customViewers.length > 0 && (
                  <div className="mt-3 flex flex-wrap gap-2">
                    {customViewers.map((person, index) => (
                      <div
                        key={index}
                        className="flex items-center gap-2 rounded-full bg-[#EDE4D3] px-3 py-1 text-sm"
                      >
                        <span>{person.full_name}</span>
                        <button
                          type="button"
                          onClick={() =>
                            setCustomViewers(customViewers.filter((_, i) => i !== index))
                          }
                          className="text-[#6B5B45] hover:text-red-600"
                        >
                          ×
                        </button>
                      </div>
                ...
                <p className="mt-2 text-[11px] text-[#9C8B75]">
                  Ketik nama untuk mencari anggota dari keluarga besar Anda.
                </p>
              </div>
            )}
            ...
            <button
              type="button"
              onClick={() => setStep(2)}
              disabled={scopeType === "custom" && customViewers.length === 0}
              className="w-full rounded-xl bg-[#4A7C59] text-white py-3 text-sm font-medium hover:bg-[#2E5239] transition disabled:opacity-40 disabled:cursor-not-allowed"
            >
          ...
          <div className="bg-white border border-[#D4C4A8] rounded-2xl p-6 space-y-6 mb-6 shadow-sm">
            ...
            <div>
              <div
                ...
                className={`relative rounded-2xl border-2 border-dashed p-8 text-center transition ${
                  isCompressing
                    ? "border-[#D4C4A8] bg-[#F5F0E8] cursor-wait"
                    : isDragActive
                    ? "border-[#4A7C59] bg-[#F5F0E8] cursor-pointer"
                    : "border-[#D4C4A8] cursor-pointer"
                }`}
              >
                  ...
                  {mediaList.length < MAX_FILES && (
                    <div
                      ...
                      className={`flex aspect-square items-center justify-center rounded-xl border-2 border-dashed ${
                        isCompressing
                          ? "border-[#D4C4A8] bg-[#F5F0E8] cursor-wait opacity-60"
                          : "border-[#D4C4A8] hover:bg-[#F5F0E8] cursor-pointer"
                      }`}
                    >
                      {isCompressing ? (
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                  allowComment ? "bg-[#4A7C59]" : "bg-[#D4C4A8]"
                }`}
              >
                <span
                  className={`inline-block h-5 w-5 transform rounded-full bg-white shadow transition-transform ${
                    allowComment ? "translate-x-5" : "translate-x-0.5"
                  }`}
                />
              </button>
            </div>
            ...
            <div className="rounded-xl bg-[#FDFAF5] border border-[#D4C4A8] p-4 space-y-2 text-sm">
              <span className="bg-[#EDE4D3] text-[#3B2F1E] rounded-full px-3 py-0.5 text-xs inline-block">
                {scopeType === "extended" && "Keluarga Besar"}
                {scopeType === "nuclear" && "Keluarga Inti"}
                {scopeType === "custom" && "Pilih Manual"}
              </span>
              {mediaList.length > 0 && (
                <p className="text-[#6B5B45]">{mediaList.length} foto dipilih</p>
              )}
            ...
            <div className="mt-4 flex items-center justify-between">
              <button
                onClick={() => setStep(1)}
                className="rounded-xl bg-[#D4C4A8] px-6 py-3 text-sm font-medium hover:bg-[#C4B39A] transition"
              >
                ← Kembali
              </button>
              <Button
