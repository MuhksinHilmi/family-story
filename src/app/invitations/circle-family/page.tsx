...
export default function HalaqahInvitationsPage() {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener('halaqah-invitations-updated', handler);
  }, []);
  ...
  const handleAction = async (invitationId: number, action: 'accept' | 'reject') => {
    try {
      ...
      if (res.ok) {
        toast.success(`Undangan berhasil ${action === 'accept' ? 'diterima' : 'ditolak'}`);
        fetchInvitations();
  ...
  return (
    <div className="min-h-screen bg-[#F5F0E8] p-6">
      <div className="max-w-4xl mx-auto space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-[#3B2F1E]">Undangan Circle Family</h1>
          <p className="text-[#6B5B45]">Tinjau undangan dari keluarga lain untuk bergabung bersama.</p>
        </div>
        ...
        {invitations.length > 0 ? (
                ...
                <Card className="bg-[#FDFAF5] border-[#D4C4A8]">
                  <CardContent className="p-5 space-y-4">
                    <div className="flex items-start justify-between">
                      <div>
                        <h3 className="font-bold text-[#3B2F1E] text-lg">{inv.halaqah_name}</h3>
          ...
          <div className="text-center py-20 space-y-4">
            ...
            <h3 className="text-lg font-bold text-[#3B2F1E]">Tidak Ada Undangan</h3>
            <p className="text-[#6B5B45]">Anda tidak memiliki undangan Circle Family yang pending.</p>
