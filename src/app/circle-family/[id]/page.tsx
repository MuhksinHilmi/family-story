...
export default function HalaqahDetailPage() {
  ...
  if (isLoading || !data) {
    return <FamilyTreeLoader message="Memuat detail circle family..." />;
  }
      ...
      <div className="bg-[#FDFAF5] border-b border-[#D4C4A8] sticky top-0 z-20">
        <div className="max-w-5xl mx-auto px-4 py-4 flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-[#3B2F1E] leading-tight">
                {halaqah.name}
              </h1>
              <div className="flex items-center gap-2">
                <Badge
                  variant="outline"
                  className="text-[10px] border-[#D4C4A8] text-[#6B5B45]"
                >
                  {halaqah.type}
                </Badge>
                <span className="text-xs text-[#9C8B75]">
                  • {halaqah.location_label}
                </span>
              </div>
            </div>
          </div>
            ...
            >
              <Card className="bg-[#FDFAF5] border-[#D4C4A8]">
                ...
                <CardContent className="space-y-4">
                  <p className="text-[#6B5B45] leading-relaxed">
                    {halaqah.description}
                  </p>
                  <div className="flex flex-wrap gap-2">
                    {halaqah.topics?.map((t: string) => (
                      <Badge
                        key={t}
                        className="bg-[#EDE4D3] text-[#6B5B45] border-none text-xs"
                      >
                        {t}
                      </Badge>
                    ))}
              ...
              {learningPath ? (
                ...
                <div className="text-center py-20 bg-[#FDFAF5] rounded-3xl border-[#D4C4A8] border-2 space-y-4">
                  ...
                  <p className="text-[#6B5B45]">
                    Belum ada kurikulum belajar untuk circle family ini.
                  </p>
                  <Button
                    variant="outline"
                ...
                <Card key={m.id} className="bg-[#FDFAF5] border-[#D4C4A8]">
                  <CardContent className="p-4 flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      <p className="font-bold text-[#3B2F1E] text-sm">
                        {m.full_name}
                      </p>
                      <p className="text-xs text-[#9C8B75]">
                        {m.family_name} • {m.occupation}
                      </p>
                    </div>
                    {m.is_admin && (
                      <Badge className="bg-[#C4922A] text-white">Admin</Badge>
                    )}
                  </CardContent>
                </Card>
              ))}
            ...
            >
              <div className="bg-[#FDFAF5] border border-[#D4C4A8] rounded-2xl h-full flex items-center justify-center text-[#9C8B75]">
                ...
                <div className="text-center space-y-2">
                  ...
                  <p>
                    Koneksi ke Ruang Diskusi... (Room UUID:{" "}
                    {halaqah.chat_room_uuid})
                  </p>
                </div>
              </div>
            </motion.div>
